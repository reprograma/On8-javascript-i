...
botaoCalcular.addEventListener('click', function (event) {
  ...
  mensagem.textContent = `Parabéns! Você foi aprovado com a média é ${mediaFinal}`;
   ...
   if(mediaFinal < 7 && mediaFinal >= 0) {
      mensagem.textContent = `Que pena, sua nota foi baixa. Sua nota é ${mediaFinal}`; 
     ...
     if (isNaN(nota1) || isNaN(nota2)) {
        mensagem.textContent ='Preencha corretamente os campos, com valores númericos entre 0 e 10';
...
botaoApagar.eddEventListenner('click', function(event){
event.preventDefault();
